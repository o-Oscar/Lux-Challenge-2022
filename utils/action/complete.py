...
class CompleteActionHandler(BaseActionHandler):
    def __init__(self):
        super().__init__()
    ...
    def calc_masks(self, obs):
        ...
        for team in teams:
            ...
            for unit_name, unit in obs[team]["units"][team].items():
                ...
                if factory_mask[unit["pos"][0], unit["pos"][1]] == 0:
                    mask[7] = 0
                    mask[8] = 0
                    mask[9] = 0
                # digging not possible when on top of factory or not on top of a square with rubble, ice or ore
                if (
                    obs[team]["board"]["ice"][unit["pos"][0], unit["pos"][1]] == 0
                    and obs[team]["board"]["ore"][unit["pos"][0], unit["pos"][1]] == 0
                    and obs[team]["board"]["rubble"][unit["pos"][0], unit["pos"][1]]
                    == 0
                ):
                    mask[10] = 0
                if factory_mask[unit["pos"][0], unit["pos"][1]] == 1:
                    mask[10] = 0
