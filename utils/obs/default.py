...
#             if len(all_deltas) > 0:
#                 full_grid[2 + i] = np.min(all_deltas, axis=0) / (2 * 48)

#         # time in the day
#         full_grid[4] = np.sin(np.pi * 2 * obs[team]["real_env_steps"] / 50) / (11**2)
#         full_grid[5] = np.cos(np.pi * 2 * obs[team]["real_env_steps"] / 50) / (11**2)
